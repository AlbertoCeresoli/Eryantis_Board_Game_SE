...
public class Controller {
    ...
    public void round() throws InterruptedException {
        ...
        playerOrder = planningPhase(cards);

        //action phase
        //for the number of players
        Colors color = null;

        actionPhase(cards, playerOrder, color);
    }
    ...
    /**
     * This iterates once per round asking for the assistant card each player wants to play, then an array containing the order of the action phase is created and returned to the caller
     * @param cards array which contains the state of every assistant card: -1. not used 0. used this turn 2. used
     * @return  the order of the action phase
     * @throws InterruptedException
     */
     private int[] planningPhase(int[] cards) throws InterruptedException {
        int[] playerOrder;
        //play assistant card
        for (int i = 0; i < model.gameRules[0]; i++) {
            cards[i] = -1;
