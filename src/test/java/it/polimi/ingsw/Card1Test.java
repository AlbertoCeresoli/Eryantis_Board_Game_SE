...
public class Card1Test {
	...
	void useEffectTest() throws StudentNotAvailableException, OutOfBoundException, WrongArrayException {
		...
		int[] temp = card1.getIslandInteraction().getIslands().get(index).getStudents();

		System.arraycopy(temp, 0, oldIslandStudents, 0, Constants.NUMBER_OF_STUDENTS_COLOR);

		card1.useEffect(index, 0, students, null);

		int[] newIslandStudents = new int[Constants.NUMBER_OF_STUDENTS_COLOR];
		int[] temp2 = card1.getIslandInteraction().getIslands().get(index).getStudents();
		System.arraycopy(temp2, 0, newIslandStudents, 0, Constants.NUMBER_OF_STUDENTS_COLOR);
	 ...
	 * It is thrown because the input array students is {0, 0, 0, 0, 1}, but the card's student array is initialized like {1, 1, 1, 1, 0},
	 * so we are looking for a student that we do not have
	 */
	@Test
